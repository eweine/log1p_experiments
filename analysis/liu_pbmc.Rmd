---
title: "Single Cell PBMC Analysis"
output:
...
Here, I will apply the `log1p` matrix factorization model to a dataset of approximately $10K$ single cells labeled with FACS sorting from a single donor.
...
I filtered out genes that are not expressed in at least $5$ cells. This leaves $17642$ remaining genes.
...
fit_list <- readr::read_rds(
  "~/Documents/data/passPCA/liu_results/fit_list.rds"
  )
```
...
```{r}
ll_list <- readr::read_rds("~/Documents/data/passPCA/liu_results/loglik.rds")

library(ggplot2)
ggplot(data = ll_list$df) +
  ...
  scale_x_log10() +
  scale_y_log10() +
  geom_hline(yintercept = max(ll_list$ll_vec) - ll_list$nmf_ll + 1, color = "red", linetype = "dashed") +
...
```{r}
cc_out_vec <- c(
  rep("1e-4", nrow(counts) * 25),
  rep("1e-3", nrow(counts) * 25),
  rep("1e-2", nrow(counts) * 25),
  rep("1e-1", nrow(counts) * 25),
  rep("1", nrow(counts) * 25),
  rep("1e1", nrow(counts) * 25),
  rep("1e2", nrow(counts) * 25),
  rep("1e3", nrow(counts) * 25),
  rep("nmf", nrow(counts) * 25)
)
...
```{r}
cc_out_vec <- c(
  rep("1e-4", ncol(counts) * 25),
  rep("1e-3", ncol(counts) * 25),
  rep("1e-2", ncol(counts) * 25),
  rep("1e-1", ncol(counts) * 25),
  rep("1", ncol(counts) * 25),
  rep("1e1", ncol(counts) * 25),
  rep("1e2", ncol(counts) * 25),
  rep("1e3", ncol(counts) * 25),
  rep("nmf", ncol(counts) * 25)
)
...
Now, I show the GO terms for the top 20 genes for each factor.
...
c0001_gdf <- get_go_terms(fit_list$`1e-04`$V)
c001_gdf <- get_go_terms(fit_list$`0.001`$V)
c01_gdf <- get_go_terms(fit_list$`0.01`$V)
cp1_gdf <- get_go_terms(fit_list$`0.1`$V)
c1_gdf <- get_go_terms(fit_list$`1`$V)
c10_gdf <- get_go_terms(fit_list$`10`$V)
c100_gdf <- get_go_terms(fit_list$`100`$V)
c1000_gdf <- get_go_terms(fit_list$`1000`$V)
cnmf_gdf <- get_go_terms(fit_list$nmf$F)
```

### c = 1e-4

```{r}
knitr::kable(c0001_gdf)
```

### c = 1e-3

```{r}
knitr::kable(c001_gdf)
```
...
```{r}
knitr::kable(c100_gdf)
```

### c = 1000

```{r}
knitr::kable(c1000_gdf)
```

### nmf

```{r}
knitr::kable(cnmf_gdf)
```

I think the takeaway here is the same as for the documents. That is, in the topic model you see many of the same genes appearing as the top genes in each factor. Smaller values of $c$, however, seem to lead to each factor encoding different information. 
