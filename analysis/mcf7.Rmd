...
```{r topic-model-vs-log1pNMF, fig.height=3, fig.width=3}
L1 <- fit$L
d  <- apply(L1,2,max)
...
```{r nmf-vs-log1pNMF, fig.height=3, fig.width=3}
W <- nmf$W
k <- ncol(W)
d <- apply(W,2,max)
W <- scale_cols(W,1/d)
...
d <- apply(L,2,max)
L <- scale_cols(L,1/d)
plot(W,L[,c(1,3,4,2)],pch = 20,xlab = "NMF",ylab = "log1pNMF, c = 1")
abline(a = 0,b = 1,col = "magenta",lty = "dotted")
```
