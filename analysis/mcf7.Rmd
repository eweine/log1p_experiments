...
d <- apply(W,2,max)
W <- scale_cols(W,1/d)
structure_plot(W,grouping = samples$label,topics = c(2:4,1),
               colors = topic_colors) +
  labs(y = "membership")
```
...
previous results in one important respect: the RA samples are modeled
as a combination of only two factors (the 

```{r structure-plot-log1pNMF-c01, fig.height=2.25, fig.width=5}
topic_colors <- c("olivedrab","dodgerblue","tomato","darkblue")
L <- fit_log1p_c0.1$LL
d <- apply(L,2,quantile,probs = 0.9)
L <- scale_cols(L,1/d)
structure_plot(L,grouping = samples$label,topics = c(3,4,2,1),
               colors = topic_colors) +
  labs(y = "membership")
```

It is also a bit weird that two of the samples are a combination of
all 4 factors suggesting that they are quite different from the
others, i.e., "outliers".

The other thing that is a bit concerning is that the log-likelihood is
still changing quite a bit after 200 iterations:

```{r progress-plot-log1pNMF-c01, fig.height=4, fig.width=4}
x <- fit_log1p_c0.1$objective_trace
plot(seq(1,length(x)-1),log10(diff(x)),type = "l",lwd = 1.5,
     xlab = "iteration",ylab = "change in log-likelihood")
```
