...
Let's now try a "small" setting, $c = 0.3$:

```{r log1pNMF-c01}
fit_log1p_c0.3 <- 
  fit_poisson_log1p_nmf(counts,K = 4,cc = 0.3,loglik = "exact",
                        init_method = "rank1",
...
previous results in one important respect: the ethanol samples are now
also modeled as a combination of an "ethanol factor" and a
"TGF-$\beta$ factor".

```{r structure-plot-log1pNMF-c01, fig.height=2.25, fig.width=5}
topic_colors <- c("olivedrab","darkblue","dodgerblue","tomato")
L <- fit_log1p_c0.3$LL
...
(I also tried smaller settings of $c$ and got strange results at these
settings.)
