...
log1p_fit <- fit_poisson_log1p_nmf(
  ...
  control = list(maxiter = 250, verbose = FALSE), cc = 1
)

sp2 <- normalized_structure_plot(log1p_fit, loadings_order = 1:n_cells, grouping = grouping, gap = 10, topics = c(1, rev(2:6))) +
  theme(axis.text.x = element_text(angle = 0,hjust = 0.5, size = 12)) + ylab("Membership") + ggtitle("log1p Model (c = 1)")

set.seed(1)
log1p_fit_csmall <- fit_poisson_log1p_nmf(
  Y = Y, K = 6, loglik = "exact", init_method = "rank1",
  control = list(maxiter = 250, verbose = FALSE), cc = 1e-3
)

sp3 <- normalized_structure_plot(log1p_fit_csmall, loadings_order = 1:n_cells, grouping = grouping, gap = 10, topics = c(1, rev(2:6))) +
