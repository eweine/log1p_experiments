...
```{r structure-plot-log1p-nmf, fig.height=4.25, fig.width=6.5, results="hide", message=FALSE}
scale_cols <- function (A, b)
  t(t(A) * b)
celltype_topics <- paste0("k",c(2:3,5:6,8:12))
other_topics <- paste0("k",c(1,4,13,7))
set.seed(1)
L <- fit_log1p$LL
d <- apply(L,2,max)
L <- scale_cols(L,1/d)
